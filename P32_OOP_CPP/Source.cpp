...
bool isOpenClose(char a, char b)
{
...
void checkBrackets(char* expr)
{
	...
	for (size_t i = 0; expr[i] != '\0'; i++)
	{
		...
		{
			if (brackets.isEmpty() or !isOpenClose(brackets.peek(), bracket))
			{
				std::cout << "Problem on position: " << i << ": ";
	...
	{
		std::cout << "Problem: opened bracket is not closed" << std::endl;
	}
}

int main()
{
	char expr[] = "({x-y-z}*[x+2y]-(z+4x));";
	checkBrackets(expr);
